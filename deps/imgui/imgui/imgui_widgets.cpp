...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled, bool close_on_select)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // We use ImGuiSelectableFlags_NoSetKeyOwner to allow down on one menu item, move, up on another.
    const ImGuiSelectableFlags selectable_flags = ImGuiSelectableFlags_SelectOnRelease | ImGuiSelectableFlags_NoSetKeyOwner | ImGuiSelectableFlags_SetNavIdOnHover | (close_on_select ? 0 : ImGuiSelectableFlags_DontClosePopups);
    const ImGuiMenuColumns* offsets = &window->DC.MenuColumns;
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool* p_selected, bool enabled, bool dont_close)
{
    if (MenuItemEx(label, NULL, shortcut, p_selected ? *p_selected : false, enabled, !dont_close))
    {
        if (p_selected)
            *p_selected = !*p_selected;
        return true;
